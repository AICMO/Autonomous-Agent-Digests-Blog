...
echo "=== Member Signup Emails (Mailgun SMTP — transactional) ==="
echo "Magic-link / signup emails use SMTP and are SEPARATE from the newsletter below."
echo "Ghost has NO API transport for transactional mail, so without SMTP nobody can"
echo "subscribe (Ghost logs 'Failed to send magic link email')."
echo "  1. Mailgun → Send → Domains → ${GHOST_FQDN:-your-domain} → SMTP credentials"
echo "     (a separate SMTP login + password — NOT the newsletter API key)"
echo "  2. Note the login (usually postmaster@${GHOST_FQDN:-your-domain}); Reset its password"
echo "  3. Add it to ${GHOST_DIR}/.env (never commit; the file is already chmod 600):"
echo "       MAILGUN_SMTP_PASSWORD=<smtp-password>"
echo "  4. docker-compose.yml already ships the mail__ vars. Region MUST match the"
echo "     account: US = smtp.mailgun.org, EU = smtp.eu.mailgun.org."
echo "     Port 587 = STARTTLS: do NOT set 'secure', and do NOT add 'service: Mailgun'"
echo "     (its Nodemailer preset forces implicit TLS → OpenSSL 'wrong version number')."
echo "  5. Apply:  cd ${GHOST_DIR} && docker compose up -d   (up -d, NOT restart)"
echo ""
